...
%==============================================================================
\begin{abstract}
This paper presents a regime-adaptive portfolio framework combining Hierarchical Risk Parity (HRP) with Hidden Markov Model (HMM) regime detection and XGBoost prediction. Using CRSP data (1960--2024), we address the fundamental challenge of covariance estimation with monthly data: a 60-month lookback window is insufficient to estimate a stable covariance matrix for hundreds of individual stocks ($N \gg T$). Our solution employs a \textbf{two-stage industry ETF approach}: (1) aggregate stocks into 12 value-weighted Fama-French industry portfolios, then (2) apply HRP to these 12 synthetic ETFs, ensuring $N=12 < T=60$ for a statistically valid, full-rank covariance matrix. A forward-only HMM filter using log returns, downside deviation, and Michigan Consumer Sentiment identifies market regimes without look-ahead bias. The regime-adaptive strategy achieves a Sharpe ratio of 0.85 vs.\ 0.56 for static HRP, after transaction costs. Critically, maximum drawdown falls from 47.1\% to 25.2\%, demonstrating significant tail risk reduction.
\end{abstract}
...
\begin{itemize}
    \item \textbf{Daily data}: Using daily returns would enable applying HRP directly to individual stocks (rather than industry aggregates) and refining features such as downside deviation with higher-frequency observations
    \item \textbf{Live validation}: Deploying the strategy with a paper trading account to validate real-world execution, slippage, and model degradation over time
    \item \textbf{Alternative defensive allocation}: Replacing the risk-free rate with a quantitative strategy that benefits from market crashes (e.g., tail-risk hedging, volatility carry) to maximize Sharpe ratio during bear regimes
